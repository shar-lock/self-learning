...
// 记录每桌的购物车
// {'desk:2':  [{amount, food, desk}], 'desk:3': []}
...
io.restaurant.on('connection', socket => {
  ...
  //socket.emit('foo', { a: 1, b: 2 })

  socket.join(restaurant) // 让连接加入一个房间
})

io.desk.on('connection', socket => {
  var desk = socket.handshake.query.desk // 'desk:1'

  console.log('desk client in', desk)
  ...
  // 服务器接收到前端新点的菜
  // 每当前端有人新增一个菜，那么后端就收到一个这个事件
  // info:  {desk, food, amount}
  socket.on('new food', info => {

    // 服务器上每桌的购物车里只存了已点的菜的信息。没有存所有菜或它们的数量
...
// 将会在landing页面请求并展示
// /desinfo?rid=5&did=8
app.get('/deskinfo', wrapper(async (req, res, next) => {
  // req.query.rid -> '5'
  // req.query.did -> '8'
...
//返回某餐厅的菜单
// /menu/restaurant/25
app.get('/menu/restaurant/:rid', wrapper(async (req, res, next) => {
  // CREATE TABLE foods (
  //   id integer primary key,
  //   rid integer not null,
  //   name string not null,
...
//   foods: [{id, amount}, {}, {}]
// }
app.post('/restaurant/:rid/desk/:did/order', wrapper(async (req, res, next) => {
  ...
  // 通过处理桌子相关的实时信息的sockio服务器
  // 向处于该桌的人发送下单成功的通知信息
  // in函数会返回在这个房间的所有连接
  io.desk.in(desk).emit('placeorder success', order)//通知其它人下单成功

  // 向餐厅厨房发送下单成功的通知，以让前端可以不刷新页面就实时更新订单
...
// 订单管理
// /restaurant/:rid/order?page=22&pagesize=20
// 如果要实现翻页，可以根据page和pagesize在下面的sql里加一个LIMIT子语句
...
.post(uploader.single('img'), wrapper(async (req, res, next) => {
  // 增加一个菜品
...
.put(uploader.single('img'), wrapper(async (req, res, next) => {
  var fid = req.params.fid
  var userid = req.cookies.userid
